...
import torch
import random
from new_dssm_adapter import DssmTrainer
...
def get_adapter_feat(xw, k=10, threshold=0.6):
...
def run_dssm_trainning(args, is_optuna=False, unsupervise_pos_examples=None):
  ...
  # 读入训练集
  pos_examples = []
  if unsupervise_pos_examples is None:
    ...
    pos_examples = [(src_ind2word[_src], trg_ind2word[_trg]) for _src, _trg in unsupervise_pos_examples]
  ...
  u, s, vt = xp.linalg.svd(xw, full_matrices=False)
  # u * s ?
  xsim = (u*s).dot(u.T)
  u, s, vt = xp.linalg.svd(zw, full_matrices=False)
  zsim = (u*s).dot(u.T)
  del u, s, vt
  xsim.sort(axis=1)
  zsim.sort(axis=1)  
  xsim = xp.concatenate((xsim[:, 0:100], xsim[:, 1500:1600], xsim[:, -100:]), axis=1)
  zsim = xp.concatenate((zsim[:, 0:100], zsim[:, 1500:1600], zsim[:, -100:]), axis=1)
  embeddings.normalize(xsim, ['unit', 'center', 'unit'])
  embeddings.normalize(zsim, ['unit', 'center', 'unit'])
  """
  ...
  with torch.no_grad():
    ...
    src_afeat = torch.from_numpy(asnumpy(x_adapter_feat))
    tgt_afeat = torch.from_numpy(asnumpy(z_adapter_feat))
  ...
  model.fit(torch_xw, torch_zw, train_set, src2negtgts, tgt2negsrcs, val_set, torch_orig_xw, torch_orig_zw, src_afeat=src_afeat, tgt_afeat=tgt_afeat)
