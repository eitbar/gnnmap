    ...
    def update_hard_neg_v2(self, similarity_x2y_cos, similarity_y2x_cos=None, similarity_x2y_csls=None, similarity_y2x_csls=None):
      ...
      if self.sample2negsrcs is not None and similarity_y2x_cos is not None:
        ...
        for tgt in self.tgt2gold:
          neg_candi = index_y2x[tgt][:neg_candi_size]
          neg_candi = [_ for _ in neg_candi if _ not in self.tgt2gold[tgt]]
          neg_candi1 = list(set(tgt2hard_neg_candi[tgt][:int(neg_candi_size // 2)] + neg_candi[:int(neg_candi_size // 2)]))
          neg_candi2 = list(set(tgt2hard_neg_candi[tgt][int(neg_candi_size // 2):] + neg_candi[int(neg_candi_size // 2):]))
          for _ in neg_candi2:
            if len(neg_candi1) >= neg_candi_size:
              break
            if _ not in neg_candi1:
              neg_candi1.append(_)
          tgt2hard_neg_candi[tgt] = neg_candi1        
...
class GDSSM(nn.Module):
    ...
    def forward(self, node_feat_src, node_feat_tgt, srcs_index, tgts_index, adapter_feat_src=None, adapter_feat_tgt=None):
        
        if adapter_feat_src is None:
          adapter_feat_src = node_feat_src
        
        if adapter_feat_tgt is None:
          adapter_feat_tgt = node_feat_tgt
        
        src_h = self.src_tower(self.src_adapter(node_feat_src, adapter_feat_src))
        tgt_h = self.tgt_tower(self.tgt_adapter(node_feat_tgt, adapter_feat_tgt))
...
class DssmTrainer:
    ...
    def _calc_r_in_csls(self, src_x, tgt_x, knn=10, src_afeat=None, tgt_afeat=None):
        src_hidden = self.model._get_hidden(src_x, "src", src_afeat)
        tgt_hidden = self.model._get_hidden(tgt_x, "tgt", tgt_afeat)
    ...
    def fit(self, src_x, tgt_x, train_set, src2negtgts, tgt2negsrcs, val_set, torch_orig_xw=None, torch_orig_zw=None, src_afeat=None, tgt_afeat=None):
        ...
        if src_afeat is not None:
          src_afeat = src_afeat.to(self.device)
        else:
          src_afeat = src_x

        if tgt_afeat is not None:
          tgt_afeat = tgt_afeat.to(self.device)
        else:
          tgt_afeat = tgt_x
        ...
        for e in range(self.epochs):
          ...
          if torch_orig_xw is not None and torch_orig_zw is not None:
            rt, rs, sim = self._calc_r_in_csls(torch_orig_xw, torch_orig_zw, src_afeat=src_afeat, tgt_afeat=tgt_afeat)
          else:
            rt, rs, sim = self._calc_r_in_csls(src_x, tgt_x, src_afeat=src_afeat, tgt_afeat=tgt_afeat)
          ...
          if self.update_neg_every_epoch > 0 and e % self.update_neg_every_epoch == 0:
            if self.loss_metric == "cos":
              ...
              sim_x2y_csls = sim * 2 - rt[:, None] - rs
              sim_y2x_csls = sim.transpose(0, 1) * 2 - rs[:, None] - rt 
              train_dataset.update_hard_neg(sim_x2y_csls, sim_y2x_csls)       
          ...
          for step, batch in enumerate(train_dataloader):
            ...
            logits_src2tgt, logits_tgt2src = model(src_x, tgt_x, srcs_index, tgts_index, src_afeat, tgt_afeat)
            ...
            if srcs_index.shape[1] > 1:
              ...
              loss = loss1
            if self.args.adapter_regular_method == "para":
              regular_loss = model.src_adapter.get_l2_penalty() + model.tgt_adapter.get_l2_penalty()
              loss = loss + self.args.adapter_regular * regular_loss
            loss.backward()
            optimizer.step()
            #scheduler.step()
            optimizer.zero_grad()
          ...
          if e % self.eval_every_epoch == 0 or e == self.epochs - 1:
            ...
            if torch_orig_xw is not None and torch_orig_zw is not None:
              acc, scores_result, tgts_result = self.eval(torch_orig_xw, torch_orig_zw, eval_src, eval_src2tgts, src_afeat, tgt_afeat)
            else:
              acc, scores_result, tgts_result = self.eval(src_x, tgt_x, eval_src, eval_src2tgts, src_afeat, tgt_afeat)
            ...
            if best_val_acc < acc:
              if torch_orig_xw is not None and torch_orig_zw is not None:
                ...
                rt, rs, sim = self._calc_r_in_csls(src_x, tgt_x, src_afeat=src_afeat, tgt_afeat=tgt_afeat)
              self.sim_for_unsuper = sim
              self.save()
              save_best_acc = acc
              best_val_acc = acc
              best_epoch = e
            #elif e - best_epoch > 15:
            #  break
    ...
    def eval(self, src_x, tgt_x, val_src, val_src2tgts, src_afeat=None, tgt_afeat=None):
      ...
      for i in range(0, len(val_src), eval_bs):
        ...
        bs_pred_result = self.predict(src_x, tgt_x, val_src[i:j], src_afeat=src_afeat, tgt_afeat=tgt_afeat)
    ...
    def predict(self, src_x, tgt_x, test_src, src2tgts_list=None, src_afeat=None, tgt_afeat=None):
        model = self.model
        model.to(self.device)
        ...
        with torch.no_grad():
          logits, _ = model(src_x, tgt_x, test_src, test_tgts, src_afeat, tgt_afeat)
          pred = logits
        return pred
