...
def calc_monolingual_adj(embedding, threshold=0, knn=0, method='cos'):
  ...
  print((adj > 0).sum())
  # norm
  neighbor_count = (adj > 0).sum(1)
  neighbor_count_norm = 1 / xp.sqrt(neighbor_count)
  neighbor_count_norm = neighbor_count_norm[:, xp.newaxis]
  neighbor_norm_matrix = neighbor_count_norm.dot(neighbor_count_norm.T)
  new_adj = adj * neighbor_norm_matrix

  return new_adj


def run_supervised_alignment(src_words, trg_words, x, z, src_indices, trg_indices, supervision = "--supervised", flag=False, x_adj=None, z_adj=None):
    ...
    # the dummy inputs are supplied via the function parameters, but they are included here so that the parser doesnt break
    # it's important that the parser doesnt break so that it correctly sets defaults for the many hyperparameters
    
    args = parser.parse_args(['--cuda', supervision,'DUMMY_DICT','DUMMY_SRC_INPUT','DUMMY_TAR_INPUT','DUMMY_SRC_OUTPUT','DUMMY_TAR_OUTPUT']) 
    ...
    args = parser.parse_args(['--cuda','--supervised','DUMMY_DICT','DUMMY_SRC_INPUT','DUMMY_TAR_INPUT','DUMMY_SRC_OUTPUT','DUMMY_TAR_OUTPUT']) 
    #args = parser.parse_args()
    ...
    for i in range(x_cos_sim.shape[0]):
      ...
      src_word2nn[src_ind2word[i]] = ', '.join([w + '(' + "%.5f" % s + ')' for w, s in list(zip(nn_word, nn_score))[:20]]) 
    ...
    for i in range(z_cos_sim.shape[0]):
      ...
      nn_score = tmp_sim[i].tolist()
      tgt_word2nn[trg_ind2word[i]] = ', '.join([w + '(' + "%.5f" % s + ')' for w, s in list(zip(nn_word, nn_score))[:20]]) 
    ...
    """
    # train的时候直接用graph信息改变原embedding，即train+graph，infer的时候也是infer+graph
    if not flag:
        x_adj = calc_monolingual_adj(x, 0.8, knn=5)
        z_adj = calc_monolingual_adj(z, 0.8, knn=5)
        ...
        new_x = x_adj.dot(x)
        new_z = z_adj.dot(z)
        embeddings.normalize(new_x, args.normalize)
        embeddings.normalize(new_z, args.normalize)
    ...
    #src_indices = []
    #trg_indices = []
    # removed the part that builds the seed dictionary as it is supplied by the function arguments above
    ...
    while True:
        ...
        if args.orthogonal or not end:  # orthogonal mapping
            ...
            if not flag and x_adj is not None and z_adj is not None:
                ...
                x_adj = calc_monolingual_adj(x, 0.65)
                z_adj = calc_monolingual_adj(z, 0.7)
                tmp_x = x_adj.dot(xw)
                tmp_z = z_adj.dot(zw)                
            else:
            """
...
def run_selflearning(args):
  ...
  #xw, zw = run_supervised_alignment(src_words, trg_words, x, z, src_indices, trg_indices, supervision = args.art_supervision, flag=True)
  #embeddings.normalize(x, ['unit', 'center', 'unit'])
  #embeddings.normalize(z, ['unit', 'center', 'unit'])
  #x_adj = calc_monolingual_adj(x, 0.8, 10)
  #z_adj = calc_monolingual_adj(z, 0.8, 10)
  #print('===')
